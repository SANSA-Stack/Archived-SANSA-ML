...
abstract class Commons(val spark: SparkSession, val sourceData1: RDD[Triple], val sourceData2: RDD[Triple],
                                         ...
                                         val thresholdObject: Double, val vocabSize: Long, val outputPath: String) extends Serializable {

@transient lazy val consoleLog: Logger = Logger.getLogger(getClass.getName)
private val PARTITIONS = 400
private val REPARTITION_NUMBER = 600
  ...
  def run(): RDD[(String, String)] = {
    ...
    val entityProfiles1 = getEntityProfiles(sourceData1)
    val entityProfiles2 = getEntityProfiles(sourceData2)

    // Similar entities matched based on subjects
    val dsSubjects1: RDD[(String, String, String, String)] = lshSubjects(entityProfiles1, entityProfiles2)
    val dsSubjects = dsSubjects1.repartition(REPARTITION_NUMBER).persist(StorageLevel.MEMORY_AND_DISK)
    ...
    val refinedObjects = getSimilarityObjects(dsPredicates)

    //Evaluate our results with groundtruth data
    val output = evaluation(refinedObjects)
  ...
  def getEntityProfiles(sourceData: RDD[Triple]): RDD[(String, String)] = {
    ...
    /**
     * val rPredicates: List[String] = List("owl:sameas", "wikiPageID", "wikiPageRevisionID", "wikiPageRevisionLink",
      ...
      * */
    val filename = "predicatesfilteration.txt"
    val removePredicates: List[String] = Source.fromFile(filename).getLines.toList  
   ...
   */
  def lshSubjects(entitesRDD1: RDD[(String, String)], entitiesRDD2: RDD[(String, String)]): RDD[(String, String, String, String)] = {
  ...
  def getSimilarPredicates(similarSubjRDD: RDD[(String, String, String, String)]): RDD[(String, List[String], String, List[String], List[String], Double)] = {
    ...
    val refinedDataPred = refinedDataSub.filter(f => f._6 >= jSimilartiyPredicate)
  ...
  def evaluation(result: RDD[(String, String, Double)]): RDD[(String, String)] = {
    val predictedRDD = result.map(f => {
      (f._1, f._2)
    })
    val teacherRDD = teacher.rdd
    val actualRDD = teacherRDD.map(f => {
      (f.get(0).toString(), f.get(1).toString())
    })
    
    //Calculate TruePostives for precision, recall and f1-measure
    val truePositives = actualRDD.intersection(predictedRDD).count
    consoleLog.info("***************************************************************************************")

    val actual = actualRDD.count()
    val results = predictedRDD.count()
    val precision = (truePositives * 100.00) / (results)
    consoleLog.info(s"Precision:  $precision")
    val recall = (truePositives * 100) / (actual)
    consoleLog.info(s"Recall:  $recall")
    val fMeasure = (2 * precision * recall) / (precision + recall)
    consoleLog.info(s"F-Measure: $fMeasure")
    consoleLog.info("***************************************************************************************")
    
    //Save the output_rdd
    predictedRDD.coalesce(1).saveAsTextFile(outputPath)
