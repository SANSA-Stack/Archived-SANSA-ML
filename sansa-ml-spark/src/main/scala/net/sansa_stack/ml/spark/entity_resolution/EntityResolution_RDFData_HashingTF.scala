...
class EntityResolution_RDFData_HashingTF(spark: SparkSession, triplesRDD1: RDD[Triple], triplesRDD2: RDD[Triple],
  ...
  def run(): RDD[(String, String)] = {
    ...
    val output = evaluation(refined_objects, teacher, output_path)
   ...
   * This api further uses map partition broadcasting the List removePredicates, to find triples containing the listed predicates in distinct_triples rdd
  ...
  def lsh_subjects(entites_RDD1: RDD[(String, String)], entities_RDD2: RDD[(String, String)]): RDD[(String, String, String, String)] = {
    ...
    val (hashfeatured_entities_Df1: DataFrame, hashfeatured_entities_Df2: DataFrame) = applyHashingTf_sub("ent_sub", "features", entities_Df1, entities_Df2)
   ...
   */
  def applyHashingTf_sub(inp_col: String, out_col: String, data1: DataFrame, data2: DataFrame): (DataFrame, DataFrame) = {
    val hashingTf = new HashingTF().setInputCol(inp_col).setOutputCol(out_col).setNumFeatures(vocab_size.toInt)
...
object EntityResolution_RDFData_HashingTF {
  def apply(spark: SparkSession, triplesRDD1: RDD[Triple], triplesRDD2: RDD[Triple],
            teacher: DataFrame, threshold_subject: Double, jsimilarity_predicate: Double,
            threshold_object: Double, vocab_size: Long, output_path: String): EntityResolution_RDFData_HashingTF = new EntityResolution_RDFData_HashingTF(spark, triplesRDD1, triplesRDD2,
