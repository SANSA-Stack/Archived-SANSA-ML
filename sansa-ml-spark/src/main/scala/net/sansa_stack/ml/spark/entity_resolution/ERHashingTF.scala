...
class ERHashingTF(spark: SparkSession, sourceData1: RDD[Triple], sourceData2: RDD[Triple],
  ...
  override def vectorise(inpCol: String, outCol: String, data1: DataFrame, data2: DataFrame): (DataFrame, DataFrame) = {
    ...
    val featuredEntitiesDf1 = hashingTf.transform(data1)
    val featuredEntitiesDf2 = hashingTf.transform(data2)
...
object ERHashingTF {
  def apply(spark: SparkSession, sourceData1: RDD[Triple], sourceData2: RDD[Triple],
            teacher: DataFrame, thresholdSubject: Double, jsimilarityPredicate: Double,
            thresholdObject: Double, vocabSize: Long, outputPath: String): ERHashingTF = new ERHashingTF(spark, sourceData1, sourceData2,
